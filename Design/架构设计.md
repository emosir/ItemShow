...
3. 在跳转到修改页和展示页前，文件可能未下载好，或者重复下载，在下载前应检查path属性是否存在，不存在才进行promise
4. 修改界面更新后前端与后端的同步问题
5. 分享功能受限，需要申请和适配
6. 团队成员编辑页选择成员的职位时，应该是多选，不应该是单选
7. 跳转到项目、团队修改界面时，解决跳转的数据可能为空的问题；同时，如果是创建，应该传送父类数据，目前并没有传送父类数据